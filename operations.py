...
tree = wordtree("words.txt", True, True)
...
async def endtimer(channel, client, wait):
    ...
    allpossiblesp = client.game.getall(tree) + ["ERROR1"+"ERROR2"+"ERROR3"]
    allpossibles = []
    for possible in allpossiblesp:
        if not (possible in allpossibles):
            allpossibles += [possible]
    for player in list(client.scores):
    ...
    await channel.send("Here are some of the words you could have had done:")
    await asyncio.sleep(2)
    for i in range(3):
        goodword = allpossibles[0]
        allpossibles = allpossibles[1:]
        while (goodword in sortedwords):
            goodword = allpossibles[0]
            allpossibles = allpossibles[1:]
        await channel.send("**"+goodword+"**")
        await asyncio.sleep(1)
    await asyncio.sleep(2)
